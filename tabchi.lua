...
_اماره رباته کیریت_
*/addsudo (id)*
_این کونیو سودو کنی_
*/remsudo (id)*
_این کونیو از سودی در بیاری_
*/bc (text)*
_ارسال پيام به مادر جنده ها_
*/fwd {all/gps/sgps/users}* (by reply)
_فوروارد پيام به همه/گروه ها/سوپر گروه ها/کاربران_
*/echo (text)*
_تکرار متن_
*/addedmsg (on/off)*
_تعیین روشن یا خاموش بودن پاسخ برای شر شن چس ممبرا_
*/setaddedmsg (text)*
_تعيين متن اد شدن چس ممبرا_
*/markread (on/off)*
...
*/addmembers*
_اضافه کردن چس ممبرانه ربات به گروه_
*/exportlinks*
_دريافت لينک هاي ک عضوشدم توسط ربات_
*/contactlist*
_دريافت مخاطبان ذخيره شده توسط ربات_
*Join* _us_ >> @mohammadbots
]]
    return text
  end
  do
    ...
    if msg.text:match("^[!/#]unblock") and is_sudo(msg) and #matches == 2 then
      ...
      return "_ممبرای ک دلت واسشون سوخته_"
    end
  end
    ...
    do
      ...
      local inline = function(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
_اماره رباتت_ :
_تعداد چس ممبران_ : ]] .. pvs .. [[

_گروه هایه رده پاین_ : ]] .. gps .. [[

_تعداد گروهایه کیری_ : ]] .. sgps .. [[

_لینک های ک عضو شدم_ : ]] .. links
  ...
  do
    ...
    if msg.text:match("^[!/#]addsudo") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. " _اینم سودو کردی واس ما شاخ شه_"
  ...
  do
    ...
    if msg.text:match("^[!/#]remsudo") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. " _زیادی شاخ بود_"
  ...
  do
    ...
    if msg.text:match("^[!/#]addedmsg") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "on" then
        ...
        return "_پیام اد شدن چس ممبرا_ #فعال _شد_"
      elseif matches[2] == "off" then
        redis:del("tabchi:" .. tabchi_id .. ":addedmsg")
        return "_پیام اد شدن چس ممبرا_ #غیرفعال _شد_"
      end
    end
  end
  do
    ...
    if msg.text:match("^[!/#]markread") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "on" then
        ...
        return "_خواندن پیام ها توسط رباته ممد_ #فعال _شد_"
      elseif matches[2] == "off" then
        redis:del("tabchi:" .. tabchi_id .. ":markread")
        return "_خواندن پیام ها توسط رباته ممد_ #غیرفعال _شد_"
      end
    end
  end
  do
...
_پیام_ :
  ...
  if msg.text:match("^[!/#]fwd all$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "_ریدی تو ربات باع کم فرو کن_"
  end
  if msg.text:match("^[!/#]fwd gps$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "_پیام شما به همه _#چس_مممبرل_ارسال_شد_"
  end
  if msg.text:match("^[!/#]fwd sgps$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "_پیامه شما به #گروهایه_کیری_ارسال_شد_"
  end
  ...
  if msg.text:match("^[!/#]fwd users$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "_پیامه شما به #ممبرات_ارسال_شد_"
  end
  do
    local matches = {
      msg.text:match("[!/#](lua) (.*)")
